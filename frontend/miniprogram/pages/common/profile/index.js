...
const { getSchoolsByKind, getSchoolName } = require("../../../utils/schoolsMock");
...
const roleFieldMap = {
  ...
  student: ["name", "studentNo", "schoolId", "grade", "studentAvailableTime", "subjects", "guardianContact", "learningGoal"],
...
const fieldMetaMap = {
  name: { label: "姓名", placeholder: "请输入姓名", type: "input", section: "basic", hint: "" },
  studentNo: { label: "学号", placeholder: "请填写学号", type: "input", section: "basic", hint: "" },
  workNo: { label: "学号/学工号", placeholder: "请填写学号或学工号", type: "input", section: "basic", hint: "" },
  school: { label: "学校", placeholder: "请输入学校", type: "input", section: "basic", hint: "" },
  ...
  grade: { label: "年级", placeholder: "请输入年级", type: "input", section: "basic", hint: "" },
  subjects: { label: "科目", placeholder: "如：数学、英语", type: "input", section: "ability", hint: "" },
  personality: { label: "性格描述", placeholder: "选填", type: "textarea", section: "ability", hint: "" },
  availableTime: { label: "可授课时间", placeholder: "", type: "input", section: "ability", hint: "" },
  guardianContact: { label: "监护人联系方式", placeholder: "选填", type: "input", section: "learning", hint: "" },
  learningGoal: { label: "学习目标", placeholder: "选填", type: "textarea", section: "learning", hint: "" },
  region: { label: "管辖区域", placeholder: "选填", type: "input", section: "region", hint: "" },
  managedStudents: { label: "代管学员", placeholder: "选填", type: "textarea", section: "region", hint: "" },
  organization: { label: "所属机构", placeholder: "选填", type: "input", section: "platform", hint: "" },
  permissionNote: { label: "备注", placeholder: "选填", type: "textarea", section: "platform", hint: "" }
};
...
function buildSections(role, form) {
  ...
  visibleFields.forEach((key) => {
    if (key === "schoolId" && (role === "student" || role === "teacher")) {
      const kind = role === "student" ? "recipient" : "support";
      const range = getSchoolsByKind(kind);
...
function isFilledKey(key, form) {
  if (key === "schoolId") {
    return !!((form.schoolId && String(form.schoolId).trim()) || (form.school && String(form.school).trim()));
  }
  if (key === "grade") {
...
Page({
  ...
  _loadFormFromStore() {
    ...
    const p = (u.phone && getByPhone(String(u.phone))) || u;
    return {
      l2Scope: p.l2Scope != null && p.l2Scope !== "" ? String(p.l2Scope) : "",
      name: (p.name && String(p.name).trim()) || (u.nickname && String(u.nickname).trim()) || "",
      studentNo: p.studentNo != null && p.studentNo !== "" ? String(p.studentNo) : "",
      ...
      studentAvailableTime: p.studentAvailableTime != null && p.studentAvailableTime !== "" ? String(p.studentAvailableTime) : "",
      ...
      guardianContact: p.guardianContact != null ? String(p.guardianContact) : "",
      learningGoal: p.learningGoal != null ? String(p.learningGoal) : "",
      region: p.region != null ? String(p.region) : "",
      managedStudents: p.managedStudents != null ? String(p.managedStudents) : "",
  ...
  syncPage(mergeForm) {
    ...
    const form = { ...fromStore, ...((mergeForm && typeof mergeForm === "object" && mergeForm) || {}) };
  ...
  onPickSchool(e) {
    ...
    const range = r === "student" ? getSchoolsByKind("recipient") : getSchoolsByKind("support");
    const one = range[ix];
    if (one) {
      f.schoolId = String(one.id);
      f.school = getSchoolName(one.id) || one.name;
    }
    this.syncPage(f);
  },
  onPickGrade(e) {
  ...
  async onSubmit() {
    ...
    if (role === "student" && visibleFields.includes("studentNo") && !String(form.studentNo || "").trim()) {
      wx.showToast({ title: "请填写学号", icon: "none" });
      return;
    }
    if (role === "teacher" && visibleFields.includes("workNo") && !String(form.workNo || "").trim()) {
      wx.showToast({ title: "请填写学号或学工号", icon: "none" });
      return;
    }
    if (visibleFields.includes("schoolId") && !(form.schoolId || "").trim()) {
      wx.showToast({ title: "请选择学校", icon: "none" });
      return;
    }
    if (visibleFields.includes("school") && !form.school.trim() && !visibleFields.includes("schoolId")) {
      wx.showToast({ title: "请填写学校", icon: "none" });
      return;
    }
    if (visibleFields.includes("grade")) {
      const g = String(form.grade || "").trim();
      if (!g || (role === "student" && g === "请选择年级")) {
        wx.showToast({ title: role === "student" ? "请选择年级" : "请填写年级", icon: "none" });
        return;
      }
    }
    if (role === "student") {
      const ps = parseTimeSelection(String(form.studentAvailableTime || ""));
      if (!isValidTimeSelection(ps.weekIds, ps.slotIds)) {
        wx.showToast({ title: "请至少选一天与时段", icon: "none" });
        return;
      }
    }
    if (role === "teacher") {
      const pa = parseTimeSelection(String(form.availableTime || ""));
      if (!isValidTimeSelection(pa.weekIds, pa.slotIds)) {
        wx.showToast({ title: "请至少选可授课的星期与时段", icon: "none" });
        return;
      }
    }
    const u = getApp().globalData.userInfo || {};
