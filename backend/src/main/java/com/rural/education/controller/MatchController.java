...
public class MatchController {
    ...
    public ApiResponse<List<TeacherRecommendationVO>> recommendations() {
    ...
    public ApiResponse<List<PendingApplicationVO>> pendingApplications() {
        Long userId = currentUserUtil.getCurrentUserId();
        return ApiResponse.success(matchService.pendingApplications(userId));
    }

    @PutMapping("/application/{applicationId}/process")
    public ApiResponse<Void> process(@PathVariable Long applicationId, @Valid @RequestBody ProcessRequest request) {
        Long userId = currentUserUtil.getCurrentUserId();
    ...
    @GetMapping("/my-pairs")
    public ApiResponse<List<PairDetailVO>> myPairs(@RequestParam(required = false) Integer status) {
        Long userId = currentUserUtil.getCurrentUserId();
    ...
    public ApiResponse<UnbindProgressVO> unbindProgress(@PathVariable Long pairId) {
        Long userId = currentUserUtil.getCurrentUserId();
        return ApiResponse.success(matchService.unbindProgress(userId, pairId));
    }

    @GetMapping("/{pairId}")
    public ApiResponse<PairDetailVO> pairDetail(@PathVariable Long pairId) {
        Long userId = currentUserUtil.getCurrentUserId();
