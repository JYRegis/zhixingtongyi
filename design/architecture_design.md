...
用户表 (user) ──┬── 管理员信息表 (admin_profile)
                ...
                └── 学生信息表 (student_profile)
                    ...
                    └── 匹配结对表 (match_pair) ──┬── 聊天消息表 (chat_message)
                                                 ├── 聊天参与者表 (chat_participant)
                                                 └── 会议表 (meeting)
```

### 3.2 关键特性
1. **数据隔离**：通过`school_id`、`region_code`实现二级管理员数据隔离（学生侧以云南地区学校体系为主）
...
- **二级管理员权限拆分**：
  - `student_manage`：本校学生注册审核、监管、时长审核、会话管理
...
- **学生资料阶段化**：学生申请阶段允许草稿（科目/时间可空），进入匹配前必须补全`grade+subjects+free_time`并切换为可配对状态
- **地域角色定位**：学生主要来自云南地区学校，志愿者主要来自上海，志愿者审核可由任意具备`teacher_audit`权限的二级管理员执行
...
2. 权重计算：根据算法配置计算各项权重分
   - 科目匹配度（默认 40%）：学生需求学科与教师擅长学科的重叠率
   - 时段匹配度（默认 60%）：学生与教师在 21 个离散时段（7天×3时段）上的交集率
3. 推荐排序：全量教师计算分数后按综合分降序排列
4. 双向选择：学生发起申请，志愿者确认
5. 权重可配：管理员可通过 /algorithm/weights 接口动态调整两项权重
```
...
- **材料留痕**：志愿者提交时长时需附带`service_desc`，可附带图片凭证数组`evidence_images`。
- **自动化消息联动**：
...
- **拒绝回滚规则**：任一方拒绝后进入`解绑已拒绝`状态并记录拒绝信息；再次发起时需清理历史确认位和拒绝信息，重新进入`解绑确认中`。
...
- **版本管理**：当前统一上下文前缀为`/api`（若后续启用版本化，再扩展为`/api/v1`）
- **错误处理**：统一错误码和错误信息格式
